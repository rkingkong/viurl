...
const createTransaction = async (userId, type, amount, description, extras = {}) => {
  const user = await User.findById(userId);
  if (!user) throw new Error('User not found');

  const newBalance = user.vtokens + amount;
  ...
  // Update user balance
  await User.findByIdAndUpdate(userId, { vtokens: newBalance });

  return { transaction, newBalance };
};

// Calculate daily bonus with streak
const calculateDailyBonus = (streak) => {
  const baseBonus = 1;
  
  // Streak bonuses
  if (streak >= 30) return baseBonus + 10; // 11 V-TKN for 30+ day streak
  if (streak >= 14) return baseBonus + 5;  // 6 V-TKN for 14+ day streak
  if (streak >= 7) return baseBonus + 3;   // 4 V-TKN for 7+ day streak
  if (streak >= 3) return baseBonus + 1;   // 2 V-TKN for 3+ day streak
  
  return baseBonus; // 1 V-TKN base
};
...
const checkMilestones = async (userId) => {
  ...
  const claimedMilestones = metadata.claimedMilestones || [];

  // Verification milestones
  const verificationMilestones = [
    { count: 10, reward: 50, name: '10 Verifications' },
    { count: 50, reward: 100, name: '50 Verifications' },
    { count: 100, reward: 250, name: '100 Verifications' },
    { count: 500, reward: 500, name: '500 Verifications' },
    { count: 1000, reward: 1000, name: '1000 Verifications' }
  ];

  for (const milestone of verificationMilestones) {
    const milestoneId = `verifications_${milestone.count}`;
    if (user.verificationsCount >= milestone.count && !claimedMilestones.includes(milestoneId)) {
      bonuses.push({
        id: milestoneId,
        type: 'verifications',
        ...milestone
  ...
  const followerMilestones = [
    { count: 100, reward: 25, name: '100 Followers' },
    { count: 500, reward: 75, name: '500 Followers' },
    { count: 1000, reward: 150, name: '1K Followers' },
    { count: 10000, reward: 500, name: '10K Followers' }
  ];

  for (const milestone of followerMilestones) {
    const milestoneId = `followers_${milestone.count}`;
    if (user.followers >= milestone.count && !claimedMilestones.includes(milestoneId)) {
      bonuses.push({
        id: milestoneId,
        type: 'followers',
        ...milestone
  ...
  const trustMilestones = [
    { score: 80, reward: 50, name: '80% Trust Score' },
    { score: 90, reward: 100, name: '90% Trust Score' },
    { score: 95, reward: 200, name: '95% Trust Score' }
  ];

  for (const milestone of trustMilestones) {
    const milestoneId = `trust_${milestone.score}`;
    if (user.trustScore >= milestone.score && !claimedMilestones.includes(milestoneId)) {
      bonuses.push({
        id: milestoneId,
        type: 'trust',
